 ...
 */
@RestController
@RequestMapping(value = "/agent/takeCash")
public class TakeCashController extends BaseController {
    ...
    @AllowAccess
    @ResponseBody
    @RequestMapping(name = "线上入款订单管理", value = "/cashList", method = RequestMethod.POST)
    public ResultInfo onlineOrderList(AgentOrderReq req, PageBounds page) {
        long start = System.currentTimeMillis();
        ResultInfo response = ResultInfo.ok();
        try {
            response.setData(takeCashService.cashList(req, page));
