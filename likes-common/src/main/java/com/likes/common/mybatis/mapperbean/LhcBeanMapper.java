...
public interface LhcBeanMapper {
     ...
     * @return
     */
    @Select("select number from lhc_lottery_sg order by year desc,issue desc limit #{issues}")
    List<String> getSg(@Param("issues") Integer issues);

    @Select("select issue, number as sg, time as date from lhc_lottery_sg where year = #{year} order by issue desc")
    List<LotterySgModel> getSgByYear(@Param("year") String year);

    @Select("select issue, number as sg, time as date from lhc_lottery_sg where year = #{year} order by issue DESC limit #{pageNo},#{pageSize}")
     ...
     * @return
     */
    @Select("select number as sg, issue, time as date from lhc_lottery_sg order by time desc limit #{issues}")
