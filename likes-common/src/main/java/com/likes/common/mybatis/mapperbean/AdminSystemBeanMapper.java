...
public interface AdminSystemBeanMapper {
     ...
     */
    @Select("select r.id as id, r.name as name, r.parent_id as pId, r.node_status as checked"
            + " from role_resource r ORDER BY resource_order ASC ")
     ...
     */
    @Select("select r.id id, r.`name` as `name`, r.parent_id parentId, r.permission permission, "
            + " r.type type, r.icon icon, r.url url, r.resource_order resourceOrder, "
            + " r.deleted isDeleted from role_resource r "
    ...
    @Select("SELECT account FROM `operater` where role_id = #{roleId}")
    List<Map<String, Object>> getCaiWuAccountByRoleId(@Param("roleId") Integer roleId);
}
