...
public class XjsscSgUtils {
    private static final Logger logger = LoggerFactory.getLogger(XjsscSgUtils.class);
    /**
     * 获取遗漏值（组选）
     * @param list 赛果集合
     * @return
     */
    public static SscMissNumDTO queryGroupMissVal(List<XjsscLotterySg> list) {
     ...
     * @return
     */
    private static Integer countMissVal(List<XjsscLotterySg> list, Integer num) {
        Integer count = 0;
    ...
    /**
     * 计算指定位置数字组选遗漏次数
     * @param xjsscLotterySgs 赛果
     * @param start 开始位置
     * @param end 结束位置
     * @return
     */
    public static Map<String, SscMissNumDTO> queryMissVal(List<XjsscLotterySg> xjsscLotterySgs, Integer start, Integer end) {
        Map<String, SscMissNumDTO> map = new HashMap<>();
        for (int i = start; i <= end; i++) {
            switch (i) {
                case 1 :
                    map.put("wan", countMissValByWei(xjsscLotterySgs, i));
                    break;

                case 2 :
                    map.put("qian", countMissValByWei(xjsscLotterySgs, i));
                    break;

                case 3 :
                    map.put("bai", countMissValByWei(xjsscLotterySgs, i));
                    break;

                case 4 :
                    map.put("shi", countMissValByWei(xjsscLotterySgs, i));
                    break;

                default :
                    map.put("ge", countMissValByWei(xjsscLotterySgs, i));
                    break;
            }
        }
        return map;
     ...
     * @return
     */
    private static SscMissNumDTO countMissValByWei(List<XjsscLotterySg> list, Integer wei) {
        SscMissNumDTO dto = new SscMissNumDTO();
        for (int i = 0; i < 10; i++) {
            Integer count = 0;
            for (XjsscLotterySg sg : list) {
