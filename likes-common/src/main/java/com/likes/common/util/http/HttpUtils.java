...
import com.likes.common.model.TaoBaoIpInfoData;
...
public abstract class HttpUtils {
    ...
    public static String getIpAddressByIp(String ip) {
        ...
        String result = null;
        try {
            result = doGet(TAOBAO_IPADDRESS_QUERY_URL + ip, null, DEFAULT_CHARSET, 500);
        } catch (Exception e) {
            logger.error("获取地址出错。ip:{},error:{}", ip, e.getMessage());
            return "";
        }
        ...
        TaoBaoIpInfoData taoBaoIpInfoDTO;
        try {
            JSONObject obj = JSON.parseObject(result);
            if (obj.getJSONObject("data") == null || "".equals(obj.getJSONObject("data").toJSONString())) {
                return "";
            }
            taoBaoIpInfoDTO = JSON.parseObject(obj.getJSONObject("data").toJSONString(), TaoBaoIpInfoData.class);
        } catch (Exception e) {
            logger.error("获取地址解析出错。ip:{},error:{}", ip, e);
            return "";
        }
        if (taoBaoIpInfoDTO == null || taoBaoIpInfoDTO.getCountry() == null) {
            return "";
        }
        String ipAddress = taoBaoIpInfoDTO.getIpAddress();
        logger.info("ip = [{}], ip地址 = [{}]", ip, ipAddress);
        return ipAddress;
    }
