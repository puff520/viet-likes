...
public class TensscUtils {
     ...
     * @return
     */
    public static Map<String, Object> todayCount(List<TensscLotterySg> sgList) {
     ...
     * @return
     */
    public static Map<String, Object> todayCount1(List<TensscLotterySg> sgList) {
        ...
        for (int i = 0; i < totalIssue; i++) {
            TensscLotterySg sg = sgList.get(i);
            Integer wan = sg.getWan();
            Integer qian = sg.getQian();
            Integer bai = sg.getBai();
            Integer shi = sg.getShi();
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiKaiJiang(List<TensscLotterySg> sgList) {
        // 创建结果容器
        ...
        for (TensscLotterySg tensscSg : sgList) {
            list = new ArrayList<>();
            map = new HashMap<>();
            map.put("issue", tensscSg.getIssue().substring(8));
            list.add(tensscSg.getWan() == null ? "" : tensscSg.getWan() + "," + tensscSg.getQian() + "," + tensscSg.getBai() + "," + tensscSg.getShi() + "," + tensscSg.getGe());
            list.add(CQSSCPlayRule.daxiaoDanShuang(tensscSg.getShi()));
            list.add(CQSSCPlayRule.daxiaoDanShuang(tensscSg.getGe()));
            list.add(CQSSCPlayRule.houSan(tensscSg.getBai(), tensscSg.getShi(), tensscSg.getGe()));
            map.put("list", list);
            result.add(map);
        }
        return result;
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiSg(List<TensscLotterySg> sgList) {
        if (sgList == null || sgList.size() == 0) {
        ...
        for (int i = 0; i < totalIssue; i++) {
            TensscLotterySg tensscSg = sgList.get(i);
            Map<String, Object> map = new HashMap<>();
            map.put("issue", tensscSg.getIssue());
            map.put("time", tensscSg.getTime());
            map.put("number", new StringBuffer().append(tensscSg.getWan()).append(tensscSg.getQian()).append(tensscSg.getBai())
                    .append(tensscSg.getShi()).append(tensscSg.getGe()).toString());
            map.put("numberstr", new StringBuffer().append(tensscSg.getWan()).append(tensscSg.getQian()).append(tensscSg.getBai())
                    .append(tensscSg.getShi()).append(tensscSg.getGe()).toString());
            map.put("he", tensscSg.getWan() + tensscSg.getQian() + tensscSg.getBai() + tensscSg.getShi() + tensscSg.getGe());
            result.add(map);
        }
        return result;
    }
     ...
     * @return
     */
    public static Map<String, Object> sgDetails(TensscLotterySg sg) {
        if (sg == null) {
            return null;
        }
        Integer wan = sg.getWan();
     ...
     * @return
     */
    public static List<MapListVO> lishiLengRe(List<TensscLotterySg> tensscSgs, String type) {
        if (tensscSgs == null || tensscSgs.size() == 0) {
            return new ArrayList<>(0);
        }
        int totalIssue = tensscSgs.size();
        ...
        if (LotteryInformationType.CQSSC_LSKJ_LRT_TWO.equals(type)) {
            // 二星的统计
            for (int i = 0; i < totalIssue; i++) {
                TensscLotterySg tensscSg = tensscSgs.get(i);
                Integer shi = tensscSg.getShi();
                Integer ge = tensscSg.getGe();
                if (i < 30) {
                    count30[shi] += 1;
                    count30[ge] += 1;
                } else if (i < 50) {
