...
public class XjsscUtils {
     ...
     * @return
     */
    public static Map<String, Object> todayCount(List<XjsscLotterySg> sgList) {
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiKaiJiang(List<XjsscLotterySg> sgList) {
        ...
        for (XjsscLotterySg xjsscSg : sgList) {
            sg = new ArrayList<>();
            map = new HashMap<>();
            map.put("issue", xjsscSg.getIssue().substring(8));
            sg.add(xjsscSg.getWan() == null ? "" : xjsscSg.getWan() + "," + xjsscSg.getQian() + "," + xjsscSg.getBai() + "," + xjsscSg.getShi() + "," + xjsscSg.getGe());
            sg.add(CQSSCPlayRule.daxiaoDanShuang(xjsscSg.getShi()));
            sg.add(CQSSCPlayRule.daxiaoDanShuang(xjsscSg.getGe()));
            sg.add(CQSSCPlayRule.houSan(xjsscSg.getBai(), xjsscSg.getShi(), xjsscSg.getGe()));
            map.put("list", sg);
            result.add(map);
        }
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiSg(List<XjsscLotterySg> sgList) {
        if (sgList == null || sgList.size() == 0) {
        ...
        for (int i = 0; i < totalIssue; i++) {
            XjsscLotterySg xjsscSg = sgList.get(i);
            Map<String, Object> map = new HashMap<>();
            map.put("issue", xjsscSg.getIssue());
            map.put("time", xjsscSg.getTime());
//            if(StringUtils.isEmpty(xjsscSg.getTime())){
//                map.put("time", xjsscSg.getIdealTime());
//            }else{
//                map.put("time", xjsscSg.getTime());
//            }
...
//                map.put("number", new StringBuffer().append(xjsscSg.getWan()).append(xjsscSg.getQian()).append(xjsscSg.getBai())
//                        .append(xjsscSg.getShi()).append(xjsscSg.getGe()).toString());
//                map.put("numberstr", new StringBuffer().append(xjsscSg.getWan()).append(xjsscSg.getQian()).append(xjsscSg.getBai())
//                        .append(xjsscSg.getShi()).append(xjsscSg.getGe()).toString());
//                // map.put("numberstr", CaipiaoNumberFormatUtils.NumberFormat(xjsscSg.getWan(), xjsscSg.getQian(), xjsscSg.getBai(), xjsscSg.getShi(), xjsscSg.getGe()));
//                map.put("he", xjsscSg.getWan() + xjsscSg.getQian() + xjsscSg.getBai() + xjsscSg.getShi() + xjsscSg.getGe());
//            }
            map.put("number", new StringBuffer().append(xjsscSg.getWan()).append(xjsscSg.getQian()).append(xjsscSg.getBai())
                    .append(xjsscSg.getShi()).append(xjsscSg.getGe()).toString());
            map.put("numberstr", new StringBuffer().append(xjsscSg.getWan()).append(xjsscSg.getQian()).append(xjsscSg.getBai())
     ...
     * @return
     */
    public static Map<String, Object> sgDetails(XjsscLotterySg sg) {
        if (sg == null) {
            return null;
        }
        Integer wan = sg.getWan();
     ...
     * @return
     */
    public static List<MapListVO> lishiLengRe(List<XjsscLotterySg> xjsscSgs, String type ) {
        if (xjsscSgs == null || xjsscSgs.size() == 0) {
            return new ArrayList<>(0);
        }
        int totalIssue = xjsscSgs.size();
        ...
        if (LotteryInformationType.CQSSC_LSKJ_LRT_TWO.equals(type)) {
            // 二星的统计
            for (int i = 0; i < totalIssue; i++) {
                XjsscLotterySg xjsscSg = xjsscSgs.get(i);
                Integer shi = xjsscSg.getShi();
                Integer ge = xjsscSg.getGe();
                if (i < 30) {
                    count30[shi] += 1;
                    count30[ge] += 1;
                } else if (i < 50) {
     ...
     * @return
     */
    public static Map<String,Object>  todayCount1(List<XjsscLotterySg> sgList){
        if(sgList==null || sgList.size()==0){
            return null;
        }
        ...
        int totalIssue = sgList.size();
        for(int i=0;i<totalIssue;i++){
            XjsscLotterySg sg = sgList.get(i);
            Integer wan = sg.getWan();
            Integer qian = sg.getQian();
            Integer bai = sg.getBai();
            Integer shi = sg.getShi();
