...
import com.likes.common.model.dto.ThereMemberListVO;
import com.likes.common.model.dto.result.AuspksGuanYaHeCountDTO;
import com.likes.common.model.dto.result.AuspksLiangMian;
...
public class AuspksUtils {
    ...
    public static Map<String, ThereMemberListVO> luzhuLiangMian(List<String> sg, String countType) {
        ...
        if (LotteryInformationType.AUSPKS_LMLZ_DX.equals(countType)) {
            //统计两面路珠之大小
            for (int i = 0; i < totalIssue; i++) {
                String number = sg.get(i);
                String[] numStr = number.split(",");
        ...
        } else if (LotteryInformationType.AUSPKS_LMLZ_DS.equals(countType)) {
            //统计两面路珠之单双
            for (int i = 0; i < totalIssue; i++) {
                String number = sg.get(i);
                String[] numStr = number.split(",");
        ...
        } else if (LotteryInformationType.AUSPKS_LMLZ_LH.equals(countType)) {
            //统计两面路珠之龙虎
            for (int i = 0; i < totalIssue; i++) {
                String number = sg.get(i);
                String[] numStr = number.split(",");
        ...
        HashMap<String, ThereMemberListVO> map = new HashMap<>();
        int len = 10;
        if (LotteryInformationType.AUSPKS_LMLZ_LH.equals(countType)) {
            len = 5;
        }
        for (int x = 0; x < len; x++) {
            map.put(num[x], new ThereMemberListVO(count[(x * 2)], count[(x * 2 + 1)], lists.get(x)));
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiSg(List<AuspksLotterySg> sgs) {
        if (sgs == null) {
            return null;
        }
        int totalIssue = sgs.size();
     ...
     * @return
     */
    public static Map<String, Object> sgDetails(AuspksLotterySg sg) {
        if (sg == null) {
            return null;
        }
     ...
     * @return
     */
    public static String nextIssue(String time, String issue) {
        long sgTime = TimeHelper.str2time(time, true);
        long nextIssueTime = AuspksUtils.nextIssueTime();
     ...
     * @return
     */
    public static Map<String, SscMissNumDTO> queryMissVal(List<AuspksLotterySg> sg, Integer start, Integer end) {
     ...
     * @return
     */
    public static Map<String, AuspksLiangMian> queryLiangMianMissVal(List<AuspksLotterySg> sg) {
        Map<String, AuspksLiangMian> map = new HashMap<>();
        if (sg == null) {
            return map;
        }
        ...
        map.put("0", new AuspksLiangMian(arr[0], arr[1], arr[2], arr[3]));
        for (int j = 1; j < 6; j++) {
            int k = 6 * j - 2;
            map.put("" + j, new AuspksLiangMian(arr[k], arr[k + 1], arr[k + 2], arr[k + 3], arr[k + 4], arr[k + 5]));
        }
        for (int j = 6; j < 11; j++) {
            int k = 4 * j + 10;
            map.put("" + j, new AuspksLiangMian(arr[k], arr[k + 1], arr[k + 2], arr[k + 3]));
        }
        return map;
    }
     ...
     * @return
     */
    public static Integer getAuspksMissingCount(int i, List<AuspksLotterySg> auspksLotterySg, int num) {
        int number = 0;
        for (int j = i; j < 200 && j < auspksLotterySg.size(); j++) {
            //开奖号码
            String[] split = auspksLotterySg.get(j).getNumber().split(",");
     ...
     * @return
     */
    public static Integer getSumOpenCount(List<AuspksLotterySg> auspksLotterySg, int number) {
        int count = 0;
        for (int i = 0; i < auspksLotterySg.size(); i++) {
            //开奖号码
            String[] split = auspksLotterySg.get(i).getNumber().split(",");
     ...
     * @return
     */
    public static Integer getNoOutOpenCount(List<AuspksLotterySg> auspksLotterySg, int number) {
        int count = 0;
        for (int i = 0; i < auspksLotterySg.size(); i++) {
            //开奖号码
            String[] split = auspksLotterySg.get(i).getNumber().split(",");
     ...
     * @return
     */
    public static Map<String, Object> queryTotal(List<AuspksLotterySg> sgList) {
        if (sgList == null) {
            return null;
        }
        ...
        for (AuspksLotterySg list : sgList) {
        ...
        AuspksGuanYaHeCountDTO heCount = new AuspksGuanYaHeCountDTO();
...
//     * @return
//     */
//    public static AuspksCountSgdx countSgDx(List<AuspksLotterySg> sgs) {
//        if (sgs == null) {
//            return null;
//        }
//
//        AuspksCountSgdx countSgdx = new AuspksCountSgdx();
//        AuspksLotterySg sg1 = sgs.get(0);
//        countSgdx.setTotalDate(DateUtils.parseDate(sg1.getTime()));
...
//    }
  ...
  /*  *//**
     ...
     * @return
     *//*
    public static AuspksCountSgds countSgDs(List<AuspksLotterySg> sgs) {
        if (sgs == null) {
            return null;
        }

        AuspksCountSgds countSgds = new AuspksCountSgds();
        AuspksLotterySg sg1 = sgs.get(0);
        countSgds.setTotalDate(DateUtils.parseDate(sg1.getTime()));
    ...
    *//**
     ...
     * @param countArr
     *//*
    public static void setCountDsData(AuspksCountSgds countSgds, int[] countArr) {
...
//     * @return
//     */
//    public static AuspksCountSglh countSgLh(List<AuspksLotterySg> sgs) {
//        if (sgs == null) {
//            return null;
//        }
//
//        AuspksCountSglh countSglh = new AuspksCountSglh();
//        AuspksLotterySg sg1 = sgs.get(0);
//        countSglh.setTotalDate(DateUtils.parseDate(sg1.getTime()));
...
//     * @param countArr
//     */
//    public static void setCountDsData(JsauspksCountSgds countSgds, int[] countArr) {
//        if (countSgds != null && countArr.length >= 20) {
//            countSgds.setOned(countArr[0]);
//            countSgds.setOnes(countArr[1]);
//            countSgds.setTwod(countArr[2]);
