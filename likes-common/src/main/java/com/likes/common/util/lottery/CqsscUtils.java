...
public class CqsscUtils {
     ...
     * @return
     */
    public static Map<String, Object> todayCount(List<CqsscLotterySg> sgList) {
     ...
     * @return
     */
    public static Map<String, Object> todayCount1(List<CqsscLotterySg> sgList) {
        ...
        for (int i = 0; i < totalIssue; i++) {
            CqsscLotterySg sg = sgList.get(i);
            Integer wan = sg.getWan();
            Integer qian = sg.getQian();
            Integer bai = sg.getBai();
            Integer shi = sg.getShi();
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiKaiJiang(List<CqsscLotterySg> sgList) {
        // 创建结果容器
        ...
        for (CqsscLotterySg cqsscSg : sgList) {
            list = new ArrayList<>();
            map = new HashMap<>();
            map.put("issue", cqsscSg.getIssue().substring(8));
            list.add(cqsscSg.getWan() == null ? "" : cqsscSg.getWan() + "," + cqsscSg.getQian() + "," + cqsscSg.getBai() + "," + cqsscSg.getShi() + "," + cqsscSg.getGe());
            list.add(CQSSCPlayRule.daxiaoDanShuang(cqsscSg.getShi()));
            list.add(CQSSCPlayRule.daxiaoDanShuang(cqsscSg.getGe()));
            list.add(CQSSCPlayRule.houSan(cqsscSg.getBai(), cqsscSg.getShi(), cqsscSg.getGe()));
            map.put("list", list);
            result.add(map);
        }
        return result;
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiSg(List<CqsscLotterySg> sgList) {
        if (sgList == null || sgList.size() == 0) {
        ...
        for (int i = 0; i < totalIssue; i++) {
            CqsscLotterySg cqsscSg = sgList.get(i);
            Map<String, Object> map = new HashMap<>();
            map.put("issue", cqsscSg.getIssue());
            map.put("time", cqsscSg.getTime());
//            if(StringUtils.isEmpty(cqsscSg.getTime())){
//                map.put("time", cqsscSg.getIdealTime());
//            }else{
//                map.put("time", cqsscSg.getTime());
//            }
...
//                map.put("number", new StringBuffer().append(cqsscSg.getWan()).append(cqsscSg.getQian()).append(cqsscSg.getBai()).append(cqsscSg.getShi()).append(cqsscSg.getGe()).toString());
//                map.put("numberstr", new StringBuffer().append(cqsscSg.getWan()).append(cqsscSg.getQian()).append(cqsscSg.getBai()).append(cqsscSg.getShi()).append(cqsscSg.getGe()).toString());
//                //map.put("numberstr", CaipiaoNumberFormatUtils.NumberFormat(cqsscSg.getWan(), cqsscSg.getQian(), cqsscSg.getBai(), cqsscSg.getShi(), cqsscSg.getGe()));
//                map.put("he", cqsscSg.getWan() + cqsscSg.getQian() + cqsscSg.getBai() + cqsscSg.getShi() + cqsscSg.getGe());
//            }
            map.put("number", new StringBuffer().append(cqsscSg.getWan()).append(cqsscSg.getQian()).append(cqsscSg.getBai()).append(cqsscSg.getShi()).append(cqsscSg.getGe()).toString());
            map.put("numberstr", new StringBuffer().append(cqsscSg.getWan()).append(cqsscSg.getQian()).append(cqsscSg.getBai()).append(cqsscSg.getShi()).append(cqsscSg.getGe()).toString());
            //map.put("numberstr", CaipiaoNumberFormatUtils.NumberFormat(cqsscSg.getWan(), cqsscSg.getQian(), cqsscSg.getBai(), cqsscSg.getShi(), cqsscSg.getGe()));
     ...
     * @return
     */
    public static Map<String, Object> sgDetails(CqsscLotterySg sg) {
        if (sg == null) {
            return null;
        }
        Integer wan = sg.getWan();
     ...
     * @return
     */
    public static List<MapListVO> lishiLengRe(List<CqsscLotterySg> cqsscSgs, String type) {
        if (cqsscSgs == null || cqsscSgs.size() == 0) {
            return new ArrayList<>(0);
        }
        int totalIssue = cqsscSgs.size();
        ...
        if (LotteryInformationType.CQSSC_LSKJ_LRT_TWO.equals(type)) {
            // 二星的统计
            for (int i = 0; i < totalIssue; i++) {
                CqsscLotterySg cqsscSg = cqsscSgs.get(i);
                Integer shi = cqsscSg.getShi();
                Integer ge = cqsscSg.getGe();
                if (i < 30) {
                    count30[shi] += 1;
                    count30[ge] += 1;
                } else if (i < 50) {
