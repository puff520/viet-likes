 ...
 *
 * @author sheng
 * @create 2019-03-09 21:12
 **/
public class TjsscUtils {
     ...
     * @return
     */
    public static Map<String, Object> todayCount(List<TjsscLotterySg> sgList) {
     ...
     * @return
     */
    public static Map<String, Object> todayCount1(List<TjsscLotterySg> sgList) {
        ...
        for (int i = 0; i < totalIssue; i++) {
        	TjsscLotterySg sg = sgList.get(i);
            Integer wan = sg.getWan();
            Integer qian = sg.getQian();
            Integer bai = sg.getBai();
            Integer shi = sg.getShi();
     ...
     * @return
     */
    public static List<Map<String, Object>> lishiKaiJiang(List<TjsscLotterySg> sgList) {
        // 创建结果容器
        ...
        for (TjsscLotterySg tjsscSg : sgList) {
            list = new ArrayList<>();
            map = new HashMap<>();
            map.put("issue", tjsscSg.getIssue().substring(8));
            list.add(tjsscSg.getWan() == null ? "" : tjsscSg.getWan() + "," + tjsscSg.getQian() + "," + tjsscSg.getBai() + "," + tjsscSg.getShi() + "," + tjsscSg.getGe());
            list.add(CQSSCPlayRule.daxiaoDanShuang(tjsscSg.getShi()));
            list.add(CQSSCPlayRule.daxiaoDanShuang(tjsscSg.getGe()));
            list.add(CQSSCPlayRule.houSan(tjsscSg.getBai(), tjsscSg.getShi(), tjsscSg.getGe()));
            map.put("list", list);
            result.add(map);
        }
        return result;
     ...
     * @return
     */                    								 
    public static List<Map<String, Object>> lishiSg(List<TjsscLotterySg> sgList) {
        if (sgList == null || sgList.size() == 0) {
        ...
        for (int i = 0; i < totalIssue; i++) {
        	TjsscLotterySg tjsscSg = sgList.get(i);
            Map<String, Object> map = new HashMap<>();
            map.put("issue", tjsscSg.getIssue());
            map.put("time", tjsscSg.getTime());
            map.put("number", new StringBuffer().append(tjsscSg.getWan()).append(tjsscSg.getQian()).append(tjsscSg.getBai())
                    .append(tjsscSg.getShi()).append(tjsscSg.getGe()).toString());
            map.put("numberstr", new StringBuffer().append(tjsscSg.getWan()).append(tjsscSg.getQian()).append(tjsscSg.getBai())
                    .append(tjsscSg.getShi()).append(tjsscSg.getGe()).toString());
            map.put("he", tjsscSg.getWan() + tjsscSg.getQian() + tjsscSg.getBai() + tjsscSg.getShi() + tjsscSg.getGe());
            result.add(map);
        }
        return result;
    }
     ...
     * @return
     */
    public static Map<String, Object> sgDetails(TjsscLotterySg sg) {
        if (sg == null) {
            return null;
        }
        Integer wan = sg.getWan();
     ...
     * @return
     */
    public static List<MapListVO> lishiLengRe(List<TjsscLotterySg> tjsscSgs, String type) {
        if (tjsscSgs == null || tjsscSgs.size() == 0) {
            return new ArrayList<>(0);
        }
        int totalIssue = tjsscSgs.size();
        ...
        if (LotteryInformationType.CQSSC_LSKJ_LRT_TWO.equals(type)) {
            // 二星的统计
            for (int i = 0; i < totalIssue; i++) {
            	TjsscLotterySg tjqsscSg = tjsscSgs.get(i);
                Integer shi = tjqsscSg.getShi();
                Integer ge = tjqsscSg.getGe();
                if (i < 30) {
                    count30[shi] += 1;
                    count30[ge] += 1;
                } else if (i < 50) {
            ...
            // 三星的统计
            for (int i = 0; i < totalIssue; i++) {
            	TjsscLotterySg tjsscSg = tjsscSgs.get(i);
                Integer bai = tjsscSg.getBai();
                Integer shi = tjsscSg.getShi();
                Integer ge = tjsscSg.getGe();
                if (i < 30) {
                    count30[bai] += 1;
                    count30[shi] += 1;
                    count30[ge] += 1;
