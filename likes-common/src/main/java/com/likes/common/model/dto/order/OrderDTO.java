...
import com.likes.common.mybatis.entity.OrderRecord;
...
import com.likes.common.util.lottery.KsUtils;
import com.likes.common.util.lottery.LhcUtils;
        ...
        try {
            ...
            for (OrderBetRecord record : orderBetList) {
                ...
                for (String playId : lotterySettingMap.keySet()) {
                    ...
                    settingPlayIdMap.put(playId, settingId);
                }
...
//    }
    ...
    public boolean getTureZhushu(OrderBetRecord record) {
        ...
        if (record.getLotteryId().toString().startsWith("11") || record.getLotteryId() == 1601 || record.getLotteryId() == 2202) {  //时时彩系列+ 腾讯分分彩 + 澳洲时时彩
        ...
        } else if (record.getLotteryId().toString().startsWith("12")) { //六合彩系列
        ...
        } else if (record.getLotteryId().toString().startsWith("13") || record.getLotteryId() == 2203) {  //pk系列 + 澳洲f1赛车
        ...
        } else if (record.getLotteryId().toString().startsWith("14")) {  //幸运飞艇系列
            //冠亚和01 ,冠亚和02,1-5名03，6-10名04，冠军05，亚军06，第3名07，第4名08，第5名09，第6名10，第7名11，第8名12，第9名13，第10名14
            String[] gyhArray = {"01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14"};
            if (ArrayUtils.toString(gyhArray).contains(String.format("%02d", record.getPlayId() % 100))) {
                zhushu = betNumber.split(",").length;
            }
        } else if (record.getLotteryId().toString().startsWith("15")) {  //pc蛋蛋系列
            //混合01 ,色波02,豹子03，特码05
        ...
        } else if (record.getLotteryId().toString().startsWith("19")) {  //牛牛系列
            //牛牛系列玩法01
            String[] lmArray = {"01"};
            if (ArrayUtils.toString(lmArray).contains(String.format("%02d", record.getPlayId() % 100))) {
                zhushu = betNumber.split(",").length;
            }
        } else if (record.getLotteryId().toString().startsWith("20")) {  //番摊系列
            //番摊系列玩法01
            String[] lmArray = {"01"};
            if (ArrayUtils.toString(lmArray).contains(String.format("%02d", record.getPlayId() % 100))) {
                zhushu = betNumber.split(",").length;
        ...
        } else if (record.getLotteryId().toString().startsWith("23")) {  //快三系列
        ...
        try {
            ...
            for (OrderBetRecord record : orderBetList) {
                ...
                if (record.getBetNumber().startsWith("_") || record.getBetNumber().contains("__") || record.getBetNumber().contains("@") || record.getPlayName().contains("@") || StringUtils.isEmpty(record.getPlayName())) {
                    logger.error("下单信息出错格式 betNumber:{},playName:{}, settingId:{}, playId:{}, clientType:{}",
                            record.getBetNumber(), record.getPlayName(), record.getSettingId(), record.getPlayId(), this.getSource());
                    return false;
                }
